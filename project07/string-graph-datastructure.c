/* Honor Pledge: I pledge on my honor that I have not given
   ...
   The purpose of this header file is to serve as the framework for the graph
   implementation used in this project. This framework consists of three
   structures: graph, edge, and vertex. The graph implementation is based on
   the adjacency list graph representation. A dynamically-allocated array
   is used to store the vertices of the graph.
 */

/* Declaring vertex because it is referred to in String_graph*/
struct vertex;
